...
class BasicDataLoader(object):
    ...
    def _load_file(self, config, data_type="train"):
        ...
        data_file = os.path.join(config['data_folder'], data_type + ".json")
    ...
    def build_rel_words(self, tokenize):
        ...
        if tokenize == 'lstm':
            ...
            if tokenize == 'bert':
                ...
                # tokenizer_name = os.path.join(base_dir, '..', 'pretrained_lms', 'sr-simbert') 
...
def load_data(config, tokenize):
    ...
    if 'sr-cwq' in config['data_folder']:
        entity2id = load_dict_int(os.path.join(config['data_folder'], config['entity2id']))
    else:
