...
class ReasonGNNLayer(BaseGNNLayer):
    ...
    def reason_layer(self, curr_dist, instruction, rel_linear, pos_emb, distances=None, dde_layer=None):
        """
        Aggregates neighbor representations with optional DDE enhancement
        """
        ...
        # DDE增强：计算几何门控
        if distances is not None and dde_layer is not None:
            geo_gate = self._compute_geometric_gate(distances, dde_layer, batch_size)
            fact_val = fact_val * geo_gate
    ...
    def _compute_geometric_gate(self, distances, dde_layer, batch_size):
        """
        计算DDE几何门控
        """
        try:
            ...
            for b in range(batch_size):
                if b in distances:
                    # 使用平均距离作为简化实现
                    avg_dist = sum(distances[b].values()) / max(len(distances[b]), 1)
                    # 简化的方向编码（实际应该基于边方向）
                    direction = 0  # 0表示正向
                    geo_gate = dde_layer(int(avg_dist), int(avg_dist), direction)
                else:
                    geo_gate = torch.ones(1, device=self.rel_features.device)
                geo_gates.append(geo_gate)
            
            # 扩展到匹配fact_val的维度
            geo_gate_tensor = torch.cat(geo_gates, dim=0)
            return geo_gate_tensor.unsqueeze(-1)  # 添加特征维度
        except Exception as e:
            # 如果DDE计算失败，返回全1门控
            return torch.ones(self.batch_rels.size(0), 1, device=self.rel_features.device)
    ...
    def forward(self, current_dist, relational_ins, step=0, return_score=False, distances=None, dde_layer=None):
        """
        Compute next probabilistic vectors and current node representations.
        Enhanced with DDE support.
        """
        ...
        for j in range(relational_ins.size(1)):
            ...
            neighbor_rep = self.reason_layer(current_dist, relational_ins[:,j,:], rel_linear, pos_emb, distances, dde_layer)
